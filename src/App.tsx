...
export default function App() {
  const [activeTab, setActiveTab] = useState<'master' | 'kalkulator' | 'katalog' | 'pengaturan'>('master');
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 pb-20 font-sans">
      ...
      {/* HEADER UTAMA DENGAN TOMBOL PENGATURAN YANG JELAS */}
      <header className="bg-emerald-700 text-white p-4 shadow-md sticky top-0 z-50 flex justify-between items-center">
        <div>
          <h1 className="text-lg font-bold tracking-wide">MarginKu Kedae Mattowa</h1>
          <p className="text-[11px] text-emerald-100">{isAdmin ? 'Mode Admin' : 'Mode Staff'}</p>
        </div>
        <div className="flex items-center gap-2">
          <button 
            onClick={() => setActiveTab('pengaturan')} 
            className="bg-emerald-800 hover:bg-emerald-900 text-white text-xs px-3 py-1.5 rounded-lg flex items-center gap-1.5 border border-emerald-600 font-medium shadow-sm">
            <Settings size={15}/> Pengaturan URL Webhook
          </button>
        </div>
      </header>
      ...
      <main className="p-4 max-w-md mx-auto space-y-4">
        ...
        {/* ================= TAB PENGATURAN (TEMPAT MENEMPELKAN URL) ================= */}
        {activeTab === 'pengaturan' && (
          <div className="space-y-4 bg-white p-4 rounded-xl border shadow-sm">
            <h2 className="text-md font-bold flex items-center gap-2 text-emerald-800"><Cloud size={20}/> Pengaturan Google Sheet Webhook</h2>
            <p className="text-xs text-slate-600 leading-relaxed">
              Tempelkan (*paste*) URL Web App Google Apps Script Anda yang berakhiran <code className="bg-slate-100 px-1 py-0.5 rounded text-emerald-700 font-mono">/exec</code> pada kotak di bawah ini:
            </p>
            <input 
              type="text" 
              ...
              className="w-full p-2.5 border text-xs rounded-lg font-mono bg-slate-50 focus:outline-none focus:ring-2 focus:ring-emerald-500" 
            />
            <div className="flex gap-2 pt-2">
              <button onClick={fetchFromCloud} className="flex-1 bg-emerald-700 hover:bg-emerald-800 text-white text-xs py-2.5 rounded-lg font-medium shadow-sm">
                Tarik Data dari Sheet
              </button>
              <button onClick={syncToCloud} className="flex-1 bg-slate-800 hover:bg-slate-900 text-white text-xs py-2.5 rounded-lg font-medium shadow-sm">
                Kirim Backup ke Sheet
              </button>
            </div>
            <button onClick={() => setActiveTab('master')} className="w-full mt-2 text-xs text-slate-500 hover:text-slate-700 underline text-center py-1">
        ...
        {activeTab === 'master' && (
          <div className="space-y-4">
            <h2 className="text-base font-bold flex items-center gap-2"><Database size={18}/> Bank Master Data</h2>
            ...
            {isAdmin && (
              <div className="space-y-4">
                ...
                <div className="bg-white p-4 rounded-xl border space-y-2">
                  ...
                  }} className="w-full bg-emerald-600 text-white text-xs py-2 rounded-lg font-medium">Simpan Bahan</button>
                </div>
                ...
                <div className="bg-white p-4 rounded-xl border space-y-2">
                  ...
                  }} className="w-full bg-emerald-600 text-white text-xs py-2 rounded-lg font-medium">Simpan Pekerja</button>
                </div>
              </div>
            )}
          </div>
        ...
        {activeTab === 'kalkulator' && isAdmin && (
          <div className="space-y-4">
            <div className="bg-white p-4 rounded-xl border shadow-sm space-y-3">
              ...
              <input type="text" placeholder="Nama Produk (cth: Dimsum)" value={namaProduk} onChange={e => setNamaProduk(e.target.value)} className="w-full p-2 border text-sm rounded-lg bg-slate-50" />
              <div className="flex items-center gap-2">
            ...
            {/* Ringkasan */}
            <div className="bg-slate-900 text-white p-4 rounded-xl space-y-4 shadow-lg border-t-4 border-emerald-500">
              <div className="flex justify-between items-center pt-1">
                <span className="text-sm font-semibold">HPP per Porsi:</span>
              ...
              <button onClick={handleSimpanResep} className="w-full bg-emerald-500 hover:bg-emerald-400 text-emerald-950 font-bold text-sm py-3 rounded-lg shadow uppercase">
                Simpan Resep HPP
              </button>
            </div>
          </div>
        ...
        {activeTab === 'katalog' && (
          <div className="space-y-4">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs bg-slate-200 px-2 py-1 rounded font-bold">{filteredResep.length} Resep</span>
            </div>
              ...
              <div key={r.id} className="bg-white p-4 rounded-xl border space-y-3">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">{r.targetPorsi} Porsi • {r.tanggalDibuat}</p>
                  </div>
                  <span className="text-sm font-bold text-emerald-700">{formatRp(r.hargaJualTarget)}</span>
                </div>
                <div className="flex gap-2 pt-2 border-t">
                  <button onClick={() => window.print()} className="flex-1 text-xs border py-2 rounded-lg flex justify-center items-center gap-1 font-medium"><Printer size={14}/> Cetak PDF</button>
                  <button onClick={() => exportToCSV(`Resep-${r.namaProduk}`, r.bahanList)} className="flex-1 text-xs border border-emerald-300 text-emerald-800 bg-emerald-50 py-2 rounded-lg flex justify-center items-center gap-1 font-medium"><Download size={14}/> Excel</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ================= NAVIGASI BAWAH ================= */}
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t flex justify-around p-2 z-50 shadow-md">
        ...
        {isAdmin && (
          <button onClick={() => setActiveTab('kalkulator')} className={`flex flex-col items-center gap-1 text-[10px] ${activeTab === 'kalkulator' ? 'text-emerald-600 font-bold' : 'text-slate-400'}`}><Calculator size={20}/> Kalkulator</button>
        )}
